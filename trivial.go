...
func (l id) Fwd(x Input) Result  { return x.Node() }
...
func (l k) Fwd(x Input) Result  { return l.Node }
...
func (l reshape) Fwd(x Input) Result {
	if err := CheckOne(x); err != nil {
		return Err{err}
	}
	return lift(gorgonia.Reshape(x.Node(), tensor.Shape(l)))
}
...
func (l dropout) Fwd(x Input) Result {
	if err := CheckOne(x); err != nil {
		return Err{err}
	}
	return lift(gorgonia.Dropout(x.Node(), float64(l)))
}
